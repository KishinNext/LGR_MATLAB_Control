...
% Last Modified by GUIDE v2.5 02-Mar-2019 23:15:32
...
function controladores (handles)
    global listacontrol  Controlador Periodo G texto4 Gz Mg Mf Wg opcion Kp Kv Ka control Cw opp opv opa Gw K int
    ...
    switch listacontrol
        case 2
                    ...
                    [K,int,Gw,Cw]=ruedas(G,Mf,Periodo,opcion,Kv,Kp,Ka);
                    Controlador=c2d(Cw,Periodo,'tustin');
                    int=tf(int);
                    int=c2d(int,Periodo,'tustin');
                    texto4=tf_to_latex(Gz*int,'G(z)');
                    axes(handles.axes3);
                    cla 'reset'
                    text('Interpreter','LaTex','string',texto4,'FontSize',18);
                    axis off
        ...
        case 3
            [K,int,num,den,Gw,Cw]=Atraso(G,Mf,Mg,Wg,Periodo,opcion,Kv,Kp,Ka);
                    %[num,den]=tfdata(Controlador);
                    Controlador=tf(num,den,Periodo);
                    int=tf(int);
                    int=c2d(int,Periodo,'tustin');
                    texto4=tf_to_latex(Controlador*int,'C(z)');
                    [~,e]=errores(Controlador,Gz,Periodo);
                    set(handles.tep,'String',e(1));
                    set(handles.tev,'String',e(2));
        ...
        case 4
            ...
            try
                if control==1   
                     [K,int,Gw,Cw]=combinacion2(G,Mf,Mg,Wg,Periodo,opcion,Kv,Kp,Ka);
                elseif control==2
                    [K,int,Gw,Cw]=combinacion1(G,Mf,Mg,Wg,Periodo,opcion,Kv,Kp,Ka);
                end 
               [num,den]=tfdata(Controlador,'v');  
                Controlador=tf(num,den,Periodo);
                    int=tf(int)
                    int=c2d(int,Periodo,'tustin');
                [~,e]=errores(Controlador*int,Gz,Periodo);
                [num,den]=tfdata(Controlador);
                Controlador=tf(num,den,Periodo);
...
function popupmenu1_Callback(hObject, eventdata, handles)
    global Zita Periodo G Gz Controlador PeriodoOriginal K int
    
    
    Gz=c2d(G,Periodo);
    controladores(handles);

        [k,e]=errores(Controlador,Gz,Periodo);
    ...
    if get(handles.Mg,'Value')==1 && get(handles.Mf,'Value')==1
        ...
        planta2=feedback(K*Gz*Controlador,1);
        datos = stepinfo(planta2,'RiseTimeLimits',[0.05 0.95]);
        set(handles.tz,'String',datos.RiseTime);
        set(handles.twn,'String',datos.Undershoot);
        set(handles.tmp,'String',datos.Overshoot);
        set(handles.tn,'String',datos.Peak);
...
function escalon_Callback(hObject, eventdata, handles)
    global Gz Controlador ValorLista4 Periodo G Polo controlador2 K int
    ...
    switch ValorLista4
        case 1
            ...
            try
                ...
                axes(handles.axes1);
                cla 'reset'
                step(feedback(K*Gz*Controlador,1));
...
function pushbutton4_Callback(hObject, eventdata, handles)
    ...
    switch ValorLista4
        case 1
            ...
            try
                planta=feedback(K*Gz*Controlador,1);
                step(planta);
                [~,t] = step(planta);
                t = 0:planta.Ts:t(end);
                axes(handles.axes1);
