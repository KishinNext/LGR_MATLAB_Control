function varargout = Frecuencia(varargin)
    gui_Singleton = 1;
...
function Frecuencia_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to Frecuencia (see VARARGIN)
    ...
    % Choose default command line output for Frecuencia
    handles.output = hObject;
    
    guidata(hObject, handles);
    
function varargout = Frecuencia_OutputFcn(hObject, eventdata, handles)
...
function Wg_Callback(hObject, eventdata, handles)
    ...
    ValorWg=get(handles.Wg,'Value');
    set(handles.popupmenu1,'enable','off')
    Vector3=[num2str(ValorMf) num2str(ValorMf)];
...
function confirmar_Callback(hObject, eventdata, handles)
    ...
    ValorZita=get(handles.Mg,'Value');
    Valorwn=get(handles.Mf,'Value');
    ValorEstablece=get(handles.ts,'Value');
    ValorSobre=get(handles.mp,'Value');
    Valorn=get(handles.Wg,'Value');
    
    set(handles.edit6,'enable','on')
    if ValorZita~=0 && Valorwn~=0
        try
            Zita=str2num(get(handles.selectMg,'String'));
            wn=str2num(get(handles.selectMf,'String'));
            ts=0;
            mp=0;
            n=0;
    ...
    elseif ValorEstablece~=0 && Valorn~=0
        try
            ...
            mp=0;
            n=str2num(get(handles.selectWg,'String'));
            if PeriodoManueal==0
                Periodo=periodoopciones(mp,ts,Zita,wn,1,G);
            else
                Periodo=Periodo
...
function popupmenu1_Callback(hObject, eventdata, handles)
    ...
    if get(handles.Mg,'Value')==1 && get(handles.ts,'Value')==1
        planta=feedback(Gz*Controlador,1);
        datos = stepinfo(planta,'RiseTimeLimits',[0.05 0.95]);
        Ts=str2double(get(handles.ets,'String'));
        set(handles.tz,'String',Zita);
    ...
    if get(handles.Wg,'Value')==1 && get(handles.ts,'Value')==1
        planta=feedback(Gz*Controlador,1);
        datos = stepinfo(planta,'RiseTimeLimits',[0.05 0.95]);
        Ts=str2double(get(handles.ets,'String'));
